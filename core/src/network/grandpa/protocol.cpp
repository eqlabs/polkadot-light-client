...
void Protocol::start() {
    ...
    auto stream = std::make_shared<libp2p::connection::LoopbackStream>(m_own_info, m_runner.getService());
...
cppcoro::task<> Protocol::incomingStreamTask(std::shared_ptr<Stream> stream) {
    ...
    if (auto self = wp.lock()) {
        if (!handshake_read_res) {
            self->m_log->trace("Can't read handshake from {}: {}",
                stream->remotePeerId().value(),
                handshake_read_res.error().message());
            stream->reset();
            co_return;
        } else {
            self->m_log->trace("Handshake received from {}", stream->remotePeerId().value());
        }
    } else {
        co_return;
    }
    ...
    if (auto self = wp.lock()) {
        if (!handshake_send_res) {
            self->m_log->trace("Can't send handshake to {}: {}",
                stream->remotePeerId().value(),
                handshake_send_res.error().message());
            stream->reset();
            co_return;
        } else {
            self->m_log->trace("Handshake send to {}", stream->remotePeerId().value());
        }
    } else {
        co_return;
    }
