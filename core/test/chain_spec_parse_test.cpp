...
public:
    ChainSpecParseTest() {
        prepareLoggers();
        chain_spec = std::make_shared<plc::core::chain::Spec>();
    }
...
protected:
    std::shared_ptr<plc::core::chain::Spec> chain_spec;
};

TEST_F(ChainSpecParseTest, ShouldParseSpec) {
    plc::core::Result<void> result = chain_spec->loadFromFile("assets/chain.json");
...
TEST_F(ChainSpecParseTest, ShouldFailOnParsingBadSpec) {
    plc::core::Result<void> result = chain_spec->loadFromFile("assets/bad_chain.json");
...
TEST_F(ChainSpecParseTest, ShouldFailOnParsingSpecWithMissingId) {
    plc::core::Result<void> result = chain_spec->loadFromFile("assets/chain_missing_id.json");
    EXPECT_TRUE(result.has_error());
    auto error = result.error();
